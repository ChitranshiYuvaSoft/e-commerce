...
const page = () => {
    ...
    const {coin, isLoading, isSuccess, isError} = useAppSelector((state:RootState) => state.coin);
    ...
    <div className="w-full h-[100%] flex items-center justify-center">
      <div className="w-[95%] h-[95%] flex items-center flex-col justify-center  flex-wrap relative overflow-x-auto shadow-md sm:rounded-lg ">
        <div className="w-full h-[10%]">
            <h2 className="text-2xl text-slate-200 font-semibold text-start">Coin Info - {coinId}</h2>
        </div>
        <div className="w-full h-[90%] flex items-center justify-center flex-col ">
          <div className="w-full h-[80%] flex items-center justify-around">
            <div className="w-[30%] h-[95%] flex items-center justify-center flex-col">
                ...
                <img src={coin?.image?.large} alt="no" className="w-[50%] h-[60%]"/>
                </div>
                <div  className="w-[100%] h-[30%]  flex items-center justify-center flex-col">
                <h3 className="text-slate-300 text-xl font-bold">Name: {coin?.name}</h3>
                <h4 className="text-slate-300 textxgl font-bold mt-1">Symbol: {coin?.symbol}</h4>
                </div>
            </div>
            <div className="w-[58%] h-[95%] bg-slate-800"></div>
          </div>
          <div className="w-full h-[20%] flex flex-col items-start justify-around px-3">
         
            <h5 className="text-slate-300 text-sm">  Current price[INR]: {coin?.market_data?.current_price?.inr}</h5>
            <h5 className="text-slate-300 text-sm">  Current price[USD]: {coin?.market_data?.current_price?.usd}</h5>
